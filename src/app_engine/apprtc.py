...
def save_message_from_client(host, room_id, client_id, message):
    ...
    while True:
        room = memcache_client.gets(key)
        logging.warning('save_message_from_client: ' + str(room))
        if room is None:
...
class MessagePage(webapp2.RequestHandler):
    ...
    def send_message_to_collider(self, room_id, client_id, message):
        ...
        if result.status_code != 200:
            ...
            self.error(500)
            return
        logging.info('urlfetch.fetch success ')
...
class JoinPage(webapp2.RequestHandler):
    ...
    def post(self, room_id):
        ...
        if room_id == constants.ROOM_REQUEST_ID:
            room_id = str(uuid.uuid1())
        logging.info("new roomId="+room_id)
...
app = webapp2.WSGIApplication([
        ('/avchat/', MainPage),
        ('/avchat/a/', analytics_page.AnalyticsPage),
        ('/avchat/compute/(\w+)/(\S+)/(\S+)', compute_page.ComputePage),
        ('/avchat/join/([a-zA-Z0-9-_]+)', JoinPage),
        ('/avchat/leave/([a-zA-Z0-9-_]+)/([a-zA-Z0-9-_]+)', LeavePage),
